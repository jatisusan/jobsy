...
const Filters = ({ selectedFilters, toggleFilter, clearAll }) => {
  ...
  return (
    <div className="rounded-2xl  p-4">
          ...
          >
            ...
            <div className="space-y-2">
              {filter.options.map((option) => {
                const filterId = `${filter.key}-${option.value}`;
                const isChecked = selectedFilters[filter.key].includes(
                  option.value,
                );
      ...
      <p className="mt-4 text-xs leading-relaxed text-text-muted">
        Fine-tune results by combining experience, location and job category.
      </p>
    </div>
  );
};
